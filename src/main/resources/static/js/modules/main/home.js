define(function(require, exports, module) {
	...
	var ChatContent = require("../chat-content/chat-content");
	...
	var chatContent = new ChatContent($(".chart-content"));
	...
	websocket.onmessage = function(event) {
		...
		setMessageInnerHTML(messageFrame.html(message.content, message.sender,
				message.type, message.from));
		promptMusic.sound();
		if (isWindowMin()) {// 窗口最小化则在桌面弹出
			notification.alert("easychart 【新消息】", message.sender + "发来："
					+ message.content);
		}
	};
	...
	function setMessageInnerHTML(innerHTML) {
		...
		if(chatContent.status == "unfocus"){
			chatContent.toBottom();
		}
	}
	...
	$(".chat-editor").on(
			...
			function(e) {
				if (send_setting == "enter" && e.keyCode == 13) {
					send();
					e.preventDefault();
				} else if (send_setting == "ctrlenter" && e.ctrlKey
						&& e.keyCode == 10) {
					send();
					e.preventDefault();
				}
	...
	function send() {
		...
		if (message != "") {
			...
			if (!$("#reciever-switch").is(':checked')) {
				messageHtml = "<a href='javascript:void(0);' style='color:#F1C40F;'>@bot</a> "
						+ message;
				message = "@bot " + message;
			} else {
				messageHtml = message;
